...
// create the three html elements that will need to be updated with js
const guessbox = document.querySelector(".guessbox");
options = guessbox.querySelector(".options");
searchInp = guessbox.querySelector("input");

window.onload = function () {
    // create a reference to CharacterData.js
...
function yesterday() {
    // get and display yesterdays character on web
    c = CD.getYesterdaysCharacter();
    let yesterday = document.querySelector(".yesterday");
    yesterday.innerHTML = `
                        <h1>Yesterday Was:</h1>
                        <div id="yesterdayCharText">${c}</div>
                        <img src="img/Character-Portraits/${c}.png"
                        onerror='this.onerror = null; this.src="img/Character-Portraits/${c}.gif"'>
                     </div>`;
}
...
function addCharacter(characters) {
    ...
    characters.forEach(char => {
        ...
        div.innerHTML = `<div class="character-select">
                               <div>
                                   <img src="img/Character-Portraits/${char[0]}.png"
                                   onerror='this.onerror = null; this.src="img/Character-Portraits/${char[0]}.gif"'>
                               </div>
                            <div>
                                <div id= "titleText">
                                    ${char[0]}
                                </div>
                                <p style="margin-bottom: 0px;margin-top: 0px;">${char[1]}</p>
                            </div>
                                <div id= "partText">
                                <p style="margin-bottom: 0px;margin-top: 0px;">${/[,]/.test(char[2]) ? 'Part(s) ' : 'Part '} ${char[2]}</p>
                        </div>
                    </div>`;
...
function addCharFromSearch() {
    // this updates the search selection. 
    // characters can be searched up by their name, parts they were in and alias name.
    let arr = [];
    let searchVal = searchInp.value.toLowerCase();
    if (searchVal != "") {
        arr = characters.filter(data => {
            return data[0].toLowerCase().split(' ').some(word => word.startsWith(searchVal.toLowerCase())) ||
                data[0].toLowerCase().startsWith(searchVal.toLowerCase()) ||
                data[1].toLowerCase().split(' ').some(word => word.startsWith(searchVal.toLowerCase())) ||
                data[1].toLowerCase().startsWith(searchVal.toLowerCase()) ||
                data[2].split(' ').some(word => word.startsWith(searchVal)) ||
                data[2].replace(/\s/g, '').replace(/,/, '').split(' ').some(word => word.startsWith(searchVal.replace(/\s/g, ''))) ||
                data[2].startsWith(searchVal.toLowerCase());
...
searchInp.addEventListener('keydown', function (event) {
    // allow use of arrow keys when searching for a character
    // hitting enter shall input the data selected
    if (event.key == 'Enter') {
...
function updateChoice(div, character) {
    // input the users selected guess and display their guess's information
    ...
    addGuessToDiv(sendGuess(character[0]), loadCharacter(character[0]))
}

function setCharacters() {
    // push all characters into an array, loaded from character_data.js
    for (let i = 0; i < CD.data.length; i++) {
        characters.push([CD.data[i][0], CD.data[i][CD.data[i].length - 1], CD.data[i][CD.data[i].length - 2]])
    }
}
...
async function addGuessToDiv(guess_data, character_data) {
    ...
    const squareContent = [
        "img/Character-Portraits/" + CD['Character'],
        CD.Gender,
        CD.Age,
        CD.Species,
        CD["Hair Colour"],
    ...
    const info = [
        "Gender", "Age", "Species", "Hair Colour",
        "Stand Type", "Occupation",
        "Nationality", "Parts In"
    ]
    ...
    squareContent.forEach(content => {
        ...
        if (i == 0) {
            ...
            img.src = content + '.png';
            img.onerror = function () { img.src = content + '.gif' };
    ...
    divs.forEach((div, index) => {
        ...
        setTimeout(() => {
            ...
            if (index == 7) {
                if (GD["PartArrow"] == 'higher') {
                    div.classList.add('square-up')
                } else if (GD["PartArrow"] == 'lower') {
                    div.classList.add('square-down')
                } else if (GD['PartArrow'] == 'undefined') {
                    div.classList.add('square-unknown')
                }
            }
...
async function loadWinModal() {
    ...
    imgdiv.innerHTML = `<img src="img/Character-Portraits/${charAnswer}.png"
    onerror='this.onerror = null; this.src="img/Character-Portraits/${charAnswer}.gif"'>`;
    ...
    spanElement3.classList.add("win-answer-guess-count")
    spanElement.textContent = new Date().toDateString();
    spanElement2.textContent = charAnswer;
    spanElement3.textContent = '#Guesses: ' + numOfGuesses;

    textdiv.appendChild(spanElement)
    textdiv.appendChild(brElement)
    textdiv.appendChild(spanElement2)
    textdiv.appendChild(brElement2)
    textdiv.appendChild(spanElement3)
    ...
    if (numOfGuesses == 1) {
        nbdiv.innerHTML = "ONE SHOT!<br> You guessed correctly with one guess!</br>";
    }
    else {
        nbdiv.innerHTML = "You Took " + numOfGuesses + " guesses to answer correctly!"
    }

    modalbody.appendChild(imgtextdiv);
    modalbody.appendChild(nbdiv);
    myModal.show();
}
